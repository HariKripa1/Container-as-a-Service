...
docker --tlsverify --tlscacert=$path"/ca.pem" --tlscert=$path"/cert.pem" --tlskey=$path"/key.pem" -H $ip_address":2376" login -u dockertestme -p pozx1230_
...
echo "Result:"
docker --tlsverify --tlscacert=$path"/ca.pem" --tlscert=$path"/cert.pem" --tlskey=$path"/key.pem" -H $ip_address":2376" service inspect $service_name
