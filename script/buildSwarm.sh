...
while [ $x -le $inst_no ]
do
    nova boot --flavor d1 --image $image_id --user-data inst-config.txt --nic net-id=$network_id --security-group default --key-name $user_name"key" $user_name"_instance_"$x
    nova_id=$(nova list | grep $user_name"_instance_"$x | awk '{print $2}')
    floating_ip=$(openstack floating ip create $public_net_name | awk '{print $4}')

    docker-machine create -d generic --generic-ssh-user ubuntu --generic-ssh-key ~/.ssh/$user_name"key" --generic-ip-address $floating_ip dm-$user_name"_instance_"$x
